...
#ifdef TX_DEBUG
#define TX_DEBUG_PRINT(fmt, args...) \
...
#define	UNMASK_INT4_INTERRUPTS			I2C_WriteByte(PAGE_0_0X72, 0x75, INTR_1_DESIRED_MASK)
#define	MASK_INT4_INTERRUPTS			I2C_WriteByte(PAGE_0_0X72, 0x75, 0x00)
...
//
static	byte	fw_power_state = POWER_STATE_FIRST_INIT;
...
#if 0	// xmoondash :: compile error
extern void rcp_cbus_uevent(u8);		//Subhransu
extern void EnableFSA9480Interrupts(void);	//Subhransu
#endif
...
{
	byte tmp;
	if( POWER_STATE_D0_MHL != fw_power_state )
	{  
		//
	...
	else if( POWER_STATE_D0_MHL == fw_power_state )
	{
...
bool SiiMhlTxDrvSendCbusCommand ( cbus_req_t *pReq  )
{
	...
	//
	if( (POWER_STATE_D0_MHL != fw_power_state ) || (0 == ReadByteCBUS(0x0a) || mscCmdInProgress))
	{
		TX_DEBUG_PRINT(("Error: Drv: fw_power_state: %02X, CBUS[0x0A]: %02X or mscCmdInProgress = %d\n",
					(int) fw_power_state,
...
void	Int1RsenIsr( void )
{
	...
	if(reg71 & BIT_5)
	{
		...
		if(rsen == 0x00)
		{
			printk(KERN_ERR "MHL Cable disconnect###\n");
			mhl_cable_status = 0;
			sii9234_cfg_power(0);
		...
		{
			printk(KERN_ERR "MHL Cable Connection ###\n");
...
static void DeglitchRsenLow(void)
{
	...
	if((I2C_ReadByte(PAGE_0_0X72, 0x09) & BIT_2) == 0x00)
	{
		...
		//
		if(POWER_STATE_D0_MHL == fw_power_state)
			//if((POWER_STATE_D0_MHL == fw_power_state)    && HalTimerExpired(TIMER_TO_DO_RSEN_DEGLITCH) )      
		{
...
static void InitCBusRegs( void )
{
	...
	I2C_WriteByte(PAGE_CBUS_0XC8, 0x8A, MHL_FEATURE_RCP_SUPPORT | MHL_FEATURE_RAP_SUPPORT);
	...
	I2C_WriteByte(PAGE_CBUS_0XC8, 0x8E, (byte) MHL_INT_AND_STATUS_SIZE);
	I2C_WriteByte(PAGE_CBUS_0XC8, 0x8F, 0);										//reserved

	// Make bits 2,3 (initiator timeout) to 1,1 for register CBUS_LINK_CONTROL_2
	regval = I2C_ReadByte(PAGE_CBUS_0XC8, (byte) REG_CBUS_LINK_CONTROL_2 );
	regval = (regval | 0x0C);
	I2C_WriteByte(PAGE_CBUS_0XC8, (byte) REG_CBUS_LINK_CONTROL_2, regval);

	// Clear legacy bit on Wolverine TX.
	regval = I2C_ReadByte(PAGE_CBUS_0XC8, (byte) REG_MSC_TIMEOUT_LIMIT);
	I2C_WriteByte(PAGE_CBUS_0XC8, (byte) REG_MSC_TIMEOUT_LIMIT, (regval & MSC_TIMEOUT_LIMIT_MSB_MASK));

	// Set NMax to 1
	I2C_WriteByte(PAGE_CBUS_0XC8, (byte) REG_CBUS_LINK_CONTROL_1, 0x01);
...
{	
	...
	// I2C method
	//
	I2C_WriteByte(PAGE_0_0X72, 0x92, (I2C_ReadByte(PAGE_0_0X72, 0x92) | 0x10));
...
void	SwitchToD0( void )
{
	...
	fw_power_state = POWER_STATE_D0_NO_MHL;
}
////////////////////////////////////////////////////////////////////
// SwitchToD3
//
...
static	void	Int4Isr( void )
{
	...
	{
		fw_power_state = POWER_STATE_D0_MHL;
		//MhlTxDrvProcessConnection();
	}
	...
	if((POWER_STATE_D3 == fw_power_state) && (reg74 & BIT_6))
	{
...
void sii9234_interrupt_event(void)
{
	...
	do {
		...
		if(((fw_power_state == POWER_STATE_D0_MHL)||(fw_power_state == POWER_STATE_D0_NO_MHL))&& mhl_cable_status)
		{
			byte tmp;
			tmp = I2C_ReadByte(PAGE_0_0X72, (0x74));   // read status
			flag |= (tmp&(BIT_2|BIT_3|BIT_4));

			printk(KERN_ERR "#1 (0x74) Temp: %x\n",(int) tmp );
			//I2C_WriteByte(PAGE_0_0X72, (0x74), tmp);   // clear all interrupts
			tmp = I2C_ReadByte(PAGE_0_0X72, 0x71);
			printk(KERN_ERR "#1 (0x71) Temp: %x\n",(int) tmp );
			//I2C_WriteByte(PAGE_0_0X72, 0x71, tmp);
			flag |= (tmp&(BIT_5|BIT_6));

			tmp = ReadByteCBUS(0x08);
			printk(KERN_ERR "#1 (ReadByteCBUS(0x08)) Temp: %x\n",(int) tmp );
			//WriteByteCBUS(0x08, tmp);
			flag |= (tmp&(BIT_2|BIT_3|BIT_4|BIT_5|BIT_6));

			tmp = ReadByteCBUS(0x1E);
			printk(KERN_ERR "#1 (ReadByteCBUS(0x1E)) Temp: %x\n",(int) tmp );
			//WriteByteCBUS(0x1E, tmp);    
			flag |= (tmp&(BIT_2|BIT_3));

			if((flag == 0xFA)||(flag == 0xFF))
				flag = 0;
		}
		if (loop_try++ > 20) {
			printk(KERN_ERR "[MHL] Interrupt event error\n");
			break;
		}
	}while(flag);
