...
static ssize_t change_switch_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	printk(KERN_ERR "[MHL_SWITCH]: operation nothing\n");
...
static DEVICE_ATTR(mhl_sel, S_IRUGO | S_IWUSR | S_IXOTH, change_switch_show, change_switch_store);
#endif
...
static void sii9234_cfg_gpio()
{
	...
	s3c_gpio_cfgpin(GPIO_MHL_WAKE_UP,GPIO_MHL_WAKE_UP_AF);//GPH1(6) XEINT 14
