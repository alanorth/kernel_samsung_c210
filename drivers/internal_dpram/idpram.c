...
static struct work_struct err_work_struct;
...
static DEFINE_SPINLOCK(mux_net_lock);
...
#define IRQ_DPRAM_INT_N		IRQ_MODEM_IF
...
static ssize_t show_loopback_value(struct device *dev, struct device_attribute *attr, char * buf)
{
	...
	printk(KERN_ERR "entered show_loopback_value \n");
	...
	total_size = (loopback_res.nTransfered *
		loopback_res.nPacketDataSize * 2); /* wrtie->read => *2 */
...
static send_loop_back_packet(const char* data, int size)
{
	...
	if(loopback_res.nTransfered == 1)
		do_gettimeofday(&loopback_res.nStartTime);


	if (loopback_ongoing) {
		pdp_mux_tty(dev, data, size);
		loopback_res.nTransfered++;
	}
...
static ssize_t store_loopback_value(struct device *dev, struct device_attribute *attr, char * buf, size_t count)
{
	...
	printk(KERN_ERR "entered store_loopback_value\n");

	if (!strncmp(buf, "start", 5)) {
		printk(KERN_ERR "Start: loopback test\n");
		...
		printk(KERN_ERR "End: loopback test\n");
	}
...
static int dpram_write(dpram_device_t *device, const unsigned char *buf, int len)
{
	...
	/* Internal DPRAM, check dpram ready?*/
	if (idpram_get_write_lock()) {
		printk(KERN_INFO "[DPRAM] %s - not ready return -EAGAIN\n", \
			__func__);
		return -EAGAIN;
	}
...
#define CLUSTER_SEGMENT	1500
	...
	if (device->serial.tty == NULL) {
		printk(KERN_ERR "[DPRAM] %s : tty device is NULL\n", __func__);
		return retval;
	}
...
static int dpram_read_fmt(dpram_device_t *device, const u16 non_cmd)
{
	...
	printk(KERN_INFO "[DPRAM] %s,  head: %d, tail: %d\n", \
		__func__, head, tail);
...
static int dpram_read_raw(dpram_device_t *device, const u16 non_cmd)
{
	...
	if (head != tail) {
		...
		if (ch == 0x7f) {
			...
			DPRAM_LOG_ERR("%s, ch :%x,  head: %d, tail: %d\n", \
				__func__, ch, head, tail);
		...
		read_offset++;	/* Debasish. To skip odd byte of header. */
		...
		if (len <= 0) {
			DPRAM_LOG_ERR("%s, wrong length, read_offset: %d, "\
				"len: %d hdr.id: %d\n", \
				__func__, read_offset, len, hdr.id);
			dpram_drop_data(device);
			return -1;
		}
...
#endif
		if (!dev) {
			DPRAM_LOG_ERR("dpram_raw_rx_data_callback : "\
				"ch_id = %u, there is no existing device.\n", \
				hdr.id);
		...
		if (DEV_TYPE_SERIAL == dev->type) {
			...
			if (!ret) {
				printk(KERN_WARNING \
					"[DPRAM] tty_insert_flip_string, " \
					"ch_id: %d, "\
					"drop byte: %d, read addr: %x\n", \
					hdr.id, size, \
					(device->in_buff_addr + \
					((u16)(tail + read_offset) % \
					device->in_buff_size)));
				dpram_drop_data(device);
				return -1;
			}
		...
		if (len & 0x01)
			read_offset++;	/*To skip odd byte of data.*/
		...
		if (ch == 0x7e) {
			read_offset++;
			read_offset++;	/* Debasish. To skip odd byte of EOF. */
		} else {
			DPRAM_LOG_ERR("Last byte: %d, "\
				"drop byte: %d, "\
				"read addr: %x\n", \
				ch, size, \
				(device->in_buff_addr + \
	...
	if (dev->id == LOOP_BACK_CHANNEL) {
		if (loopback_ongoing) {
#if 0			
			if (_memcmp(pdp_rx_buf,
				loopback_data,
				MAX_PDP_DATA_LEN)) {
				printk(KERN_ERR "received packet does NOT match\n");
			} else
#endif				
			{
			...
			printk(KERN_ERR "loopback channel got data but the test is stopped by user\n");
		}					
	}
#endif
...
{
	dpram_base = ioremap_nocache(DPRAM_START_ADDRESS_PHYS +
		DPRAM_SHARED_BANK, DPRAM_SHARED_BANK_SIZE);
	if (dpram_base == NULL) {
...
static int dpram_phone_power_on(void)
{
	 ...
	 * IOCTL->UM thread */
	DPRAM_LOG_INFO(
		"[DPRAM] power control waiting for INT_MASK_CMD_PIF_INIT_DONE\n"
		);
	...
	gpio_set_value(GPIO_PDA_ACTIVE, GPIO_LEVEL_HIGH);
	printk(KERN_INFO "[DPRAM] PDA_ACTIVE sets high.\n");

	/* Wait CP Start - 0xC8 */
	dpram_init_RetVal = wait_event_interruptible_timeout(
		dpram_init_cmd_wait_q, dpram_init_cmd_wait_condition,
		DPRAM_INIT_TIMEOUT);
	...
	RetVal = wait_event_interruptible_timeout(modem_pif_init_done_wait_q,
		modem_pif_init_wait_condition, PIF_TIMEOUT);
...
{
	u16 read;
	READ_FROM_DPRAM((void *)&read, DPRAM_PHONE2PDA_INTERRUPT_ADDRESS,
		sizeof(read));
	return read;
}
		...
		int count, int *eof, void *data)
{
	...
	p += sprintf(p,
		...
		gpio_get_value(GPIO_CP_AP_DPRAM_INT));
	len = (p - page) - off;
	if (len < 0)
		len = 0;
		...
		unsigned int cmd, unsigned long arg)
{
	...
	case DPRAM_PHONE_ON:
		...
		printk(KERN_ERR "phone on return 0, cmd=%d\n", cmd);
		return 0;
	...
	case DPRAM_CP_CRASH:
		DPRAM_LOG_ERR("dpram_tty_ioctl : DPRAM_CP_CRASH.\n");
		panic("CP Crash");
		return 0;

	default:
		printk(KERN_ERR "idpram ioctl - unkonwn cmd-%d\n", cmd);
		break;
	}
	return -ENOIOCTLCMD;
}
...
static void fmt_rcv_tasklet_handler(unsigned long data)
{
	...
	if (device && device->serial.tty) {
		...
		while (dpram_get_read_available(device)) {
			...
			if (cnt > 10) {
				DPRAM_LOG_ERR("%s, cnt is over 10(%d)\n", \
								__func__, cnt);
				dpram_drop_data(device);
				break;
			}
...
static unsigned char cpdump_debug_file_name[DPRAM_ERR_MSG_LEN] = "CP Crash";
...
static void cmd_error_display_handler(void)
{
	...
	if (dpram_phone_getstatus()) {
		READ_FROM_DPRAM(cpdump_debug_file_name,
			...
			printk(KERN_ERR "%s : debug : %s\n",
				__func__, cpdump_debug_file_name);
...
void idpram_send_force_upload(void)
{
	u16 irq_mask = INT_COMMAND(INT_MASK_CMD_ERR_DISPLAY);
...
void idpram_timeout_handler(void)
{
#ifdef _ENABLE_ERROR_DEVICE
	char buf[DPRAM_ERR_MSG_LEN];
	unsigned long flags;
	...
	if (dpram_phone_getstatus()) {
#if 0 /* tmp */
		kernel_sec_set_cause_strptr("     DPRAM  T-I-M-E-O-U-T", 25);
...
{
	printk(KERN_INFO "[DPRAM] command_handler(0x%02X)\n", cmd);
	switch (cmd & 0xFF) {
	...
	case INT_MASK_CMD_ERR_DISPLAY:
		if (!work_pending(&err_work_struct))
			schedule_work(&err_work_struct);
		break;
...
void non_command_handler(u16 non_cmd)
{
	u16 head, tail;
	/* @LDK@ formatted check. */
	...
	if (non_cmd & INT_MASK_RES_ACK_R) {
		...
		MULTIPDP_LOG_INFO("NON_CMD RES_ACK_R Interrupt ==> \
			schedule write work item\n");
		...
		if (is_net_stopped)	{
			READ_FROM_DPRAM_VERIFY(&head, \
				DPRAM_PDA2PHONE_RAW_HEAD_ADDRESS, sizeof(head));
			READ_FROM_DPRAM_VERIFY(&tail, \
				DPRAM_PDA2PHONE_RAW_TAIL_ADDRESS, sizeof(tail));

			if (head == tail) {
				if (!work_pending(&xmit_work_struct)) {
					/* we have space in out Q.  */
					/* So trigger the net interfaces */
					PREPARE_WORK(&xmit_work_struct, \
						pdp_mux_net);
					schedule_work(&xmit_work_struct);
				}
			}
...
static irqreturn_t host_wakeup_irq_handler(int irq, void *dev_id)
{
	...
	printk(KERN_DEBUG "[DPRAM] %s:AP_CP_INT(%d), PDA_ACTIVE(%d)\n", \
		__func__,
		gpio_get_value(GPIO_CP_AP_DPRAM_INT),
...
static int pdp_activate(pdp_arg_t *pdp_arg, unsigned type, unsigned flags)
{
	...
	if (type == DEV_TYPE_NET) {
		...
		if (dev->id == 7) {
			...
			MULTIPDP_LOG_INFO("%s(id: %u) serial device is created.\n", dev->vs_dev.tty_driver.name, dev->id);
		} else if (dev->id == 26) { /* skumar@wtl for ttyETS (ETS over DPRAM) */
			MULTIPDP_LOG_INFO("%s(id: %u) serial device is created.\n", dev->vs_dev.tty_driver.name, dev->id);
		} else if (dev->id == 16) {
			MULTIPDP_LOG_INFO("%s(id: %u) serial device is created.\n", dev->vs_dev.tty_driver.name, dev->id);
		} else if (dev->id == 28) {
	...
	for (loopcount = 0; loopcount < MAX_PDP_CONTEXT; loopcount++) {
		...
		if ((dev != NULL) && (dev->type == DEV_TYPE_NET)) {
			...
			if (SUSPEND == datastatus) {
				...
				if ((dev->vn_dev.netq_init != 0) &&
					(SUSPEND == dev->vn_dev.netq_active))
...
static long multipdp_ioctl(struct file *file, unsigned int cmd, unsigned char *arg)
{
...
static const struct file_operations multipdp_fops = {
	.owner =	THIS_MODULE,
...
static int pdp_mux_tty(struct pdp_info *dev, const void *data, size_t len)
{
	...
	while (count) {
		...
		if (elapsedtime_tv.tv_usec > 60000) {
			u16 head = 0, tail = 0;
			READ_FROM_DPRAM_VERIFY(&head, \
				dpram_table[RAW_INDEX].out_head_addr, \
				sizeof(head));
			READ_FROM_DPRAM_VERIFY(&tail, \
				dpram_table[RAW_INDEX].out_tail_addr, \
				sizeof(tail));
			printk(KERN_ERR \
				"[DPRAM] writing fail: %d, h(%d), t(%d)\n", \
				ret, head, tail);
...
static int pdp_mux_net(struct work_struct *data)
{
	...
	/* check current context */
	if (in_softirq_context)
		spin_lock(&mux_net_lock);
	else
		spin_lock_bh(&mux_net_lock);
	...
	while (skb) {
		...
		if (!net) {
			...
			if (in_softirq_context)
				spin_unlock(&mux_net_lock);
			else
				spin_unlock_bh(&mux_net_lock);
			return -ENODEV;
		}
	...
	if (ret > 0) {
		...
		if (ret == -ENOSPC) {
			...
			printk(KERN_INFO "[DPRAM] write nospc queue.(0x%p) "\
				"head : %d, tail : %d, "\
				"is_net: %d, freespace: %d\n",
				skb, head, tail, is_net_stopped, free_space);
...
static int multipdp_init(void)
{
	...
	ret = pdp_activate(&cplog_arg, DEV_TYPE_SERIAL, DEV_FLAG_STICKY);
	if (ret < 0) {
		MULTIPDP_LOG_ERR("failed to create a serial device for ttyCPLOGn");
		goto cplog_arg;
	}
...
{
	unsigned int dpram_irq, phone_active_irq, host_wakeup;

	dpram_irq = IRQ_DPRAM_INT_N;
	host_wakeup = gpio_to_irq(GPIO_CP_AP_DPRAM_INT);/* IRQ_HOST_WAKEUP; */
	phone_active_irq = gpio_to_irq(GPIO_PHONE_ACTIVE); /*IRQ_PHONE_ACTIVE; */
	...
	/*set_irq_type(dpram_irq, IRQ_TYPE_EDGE_FALLING);*/

	/* host wakeup gpio */
	s3c_gpio_cfgpin(GPIO_CP_AP_DPRAM_INT, S3C_GPIO_SFN(0xf));
	s3c_gpio_setpull(GPIO_CP_AP_DPRAM_INT, S3C_GPIO_PULL_NONE);
	set_irq_type(host_wakeup, IRQ_TYPE_EDGE_RISING);
	...
	/*if (gpio_is_valid(GPIO_PDA_ACTIVE)) {
		if (gpio_request(GPIO_PDA_ACTIVE, "dpram/GPIO_PDA_ACTIVE"))
			printk(KERN_ERR "request fail GPIO_PDA_ACTIVE\n");
		gpio_direction_output(GPIO_PDA_ACTIVE, GPIO_LEVEL_HIGH);
	}*/
	s3c_gpio_setpull(GPIO_PDA_ACTIVE, S3C_GPIO_PULL_NONE);
	s5p_gpio_set_drvstr(GPIO_PDA_ACTIVE, S5P_GPIO_DRVSTR_LV4);
	gpio_set_value(GPIO_PDA_ACTIVE, GPIO_LEVEL_HIGH);
}
...
static int register_interrupt_handler(void)
{
	...
	err = request_irq(dpram_irq, dpram_irq_handler, IRQF_DISABLED,
		"dpram irq", NULL);
	if (err) {
		DPRAM_LOG_ERR("DPRAM interrupt handler failed.(%d)\n", err);
		goto exit;
	}
	/* Because, we use internal dpram and it can't regist wake block
	 * I think below registration of wakeup source was not need.
	 */
	/* enable_irq_wake(dpram_irq);*/

	err = request_irq(phone_active_irq, phone_active_irq_handler,
		IRQF_DISABLED, "VIAActive", NULL);
	if (err) {
		DPRAM_LOG_ERR("Phone active interrupt handler failed.(%d)\n", err);
		free_irq(dpram_irq, NULL);
		goto exit;
	}
	enable_irq_wake(phone_active_irq);

	err = request_irq(host_wakeup, host_wakeup_irq_handler,
		IRQF_DISABLED, "Host Wakeup", NULL);
	if (err) {
		DPRAM_LOG_ERR("Host Wakeup Handler fail.(%d)\n", err);
		free_irq(phone_active_irq, NULL);
		free_irq(dpram_irq, NULL);
		goto exit;
	}
	enable_irq_wake(host_wakeup);
...
static int __devinit dpram_probe(struct platform_device *dev)
{
	...
	INIT_WORK(&err_work_struct, cmd_error_display_handler);
	multipdp_init();
...
#ifdef LOOP_BACK_TEST
	retval = device_create_file(multipdp_dev.this_device, &dev_attr_loopback);
	if (retval < 0) {
...
static int __devexit dpram_shutdown(struct platform_device *dev)
{
	int ret;
	...
	while (1) {
		if (gpio_get_value(GPIO_PHONE_ACTIVE)) {
			if (phone_wait_cnt > 5) {
				...
				gpio_set_value(GPIO_CP_RST, GPIO_LEVEL_LOW);
				gpio_set_value(GPIO_VIA_PS_HOLD_OFF, \
								GPIO_LEVEL_LOW);
			}
			if (phone_wait_cnt > 7) {
...
static int dpram_notifier_event(struct notifier_block *this,
		unsigned long event, void *ptr)
{
	int err;
	switch (event) {
	case PM_SUSPEND_PREPARE:
		printk(KERN_DEBUG "idpram PM_SUSPEND_PREPARE+\n");
		err = idpram_pre_suspend();
		if (err)
			printk(KERN_ERR "idpram pre-suspend err\n");
		break;
	case PM_POST_SUSPEND:
		printk(KERN_DEBUG "idpram PM_POST_SUSPEND+\n");
		err = idpram_post_resume();
		if (err)
			printk(KERN_ERR "idpram pre-suspend err\n");
		break;
	}
...
static struct notifier_block dpram_pm_notifier = {
	.notifier_call = dpram_notifier_event,
};
