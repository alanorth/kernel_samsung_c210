...
static int _idpram_write_lock(struct idpramctl *idpram, int lock)
{
	...
	printk(KERN_ERR "idpram write_lock(%d)\n", lock);
...
static int _idpram_pre_suspend(struct idpramctl *idpram)
{
	...
	gpio_set_value(GPIO_VIA_DPRAM_INT_N, 1);
	...
	if (!atomic_read(&idpram->read_lock)) {
		do {
			...
			/* s3c_gpio_cfgpin(GPIO_VIA_DPRAM_INT_N, S3C_GPIO_OUTPUT); */
			/* msleep(20);  */
			send_interrupt_to_phone(intr_out);
			/* s3c_gpio_cfgpin(GPIO_VIA_DPRAM_INT_N, S3C_GPIO_SFN(0x2));  */
		...
		default:
			printk(KERN_ERR "CP dpram Down not ready! intr=0x%02X\n",
...
static int _idpram_post_resume(struct idpramctl *idpram)
{
	...
	case IDPRAM_PM_DPRAM_POWER_DOWN:
		...
		gpio_set_value(GPIO_PDA_ACTIVE, GPIO_LEVEL_HIGH);
		msleep(20);
		gpio_val = gpio_get_value(GPIO_PDA_ACTIVE);
		printk(KERN_INFO "idpram PDA_ACTIVE (%d)\n", gpio_val);
		if (gpio_val == 0) {
			gpio_set_value(GPIO_PDA_ACTIVE, GPIO_LEVEL_HIGH);
			printk(KERN_INFO "idpram PDA_ACTIVE set again.\n");
		}
		break;
