...
static void usbsvn_post_resume_work(struct work_struct *work)
{
	...
	if (svn->skip_hostwakeup && mc_is_host_wakeup()) {
		dev_info(dev, "post resume host skip=%d, host gpio=%d, rpm_stat=%d",
...
resubmit:
	...
	if (page)
		netdev_free_page(dev, page);
	if (req && req->status != -ENOENT && req->status != -EPROTO)
...
int usbsvn_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
	...
	if (!share_svn)	{
		...
		if (!share_svn) {
			print(KERN_ERR "%s: netdev not registed\n", __func__);
			return -EINVAL;
		}
	}
	svn = share_svn;
	...
	if (mc_is_modem_active()) {
		usbdev->autosuspend_delay = msecs_to_jiffies(500);
		if (!svn->driver_info) {
...
skip_netdev:
	if (!svn->driver_info) {
		svn = share_svn;
		if (!svn) {
			dev_err(&intf->dev, "svnet device doesn't be allocated\n");
			err = ENOMEM;
			goto out;
		}
	}
	...
	for (i = 0; i < USBSVN_DEVNUM_MAX; i++) {
		...
		if (i == 0)	{
			...
			err = usb_driver_claim_interface(&usbsvn_driver,
										data_intf, svn);
			if (err < 0) {
...
int usbsvn_resume(struct usb_interface *intf)
{
	...
	printk(KERN_INFO "svn L0\n");

	return err;
}

static int reset_resume_cnt;
int usbsvn_reset_resume(struct usb_interface *intf)
{
	...
	reset_resume_cnt++;
	printk(KERN_INFO "svn L0-reset %d\n", reset_resume_cnt);
...
static int _netdev_init(void)
{
	...
	svn->tx_workqueue = alloc_workqueue("svnet_txq",
