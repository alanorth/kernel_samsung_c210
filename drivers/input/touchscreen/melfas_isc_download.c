    ...
    for(i=0;i<3;i++)
    {
        if(dl_enable_bit & (1<<i)) // 0: core, 1: private custom,  2: public custom
        {
			if(touch_id == 0){
				...
				pr_info("[TSP] system_rev 2~ touch_id error!");
				nRet = -1;
			}
            if (nRet)
                goto fw_error;
...
static void mcsdl_i2c_start(void)
{
    MCSDL_GPIO_SDA_SET_OUTPUT_ISC(1);mcsdl_delay(MCSDL_DELAY_1US);
    MCSDL_GPIO_SCL_SET_OUTPUT_ISC(1);mcsdl_delay(MCSDL_DELAY_1US);
...
static void mcsdl_i2c_stop(void)
{
    MCSDL_GPIO_SCL_SET_OUTPUT_ISC(0);mcsdl_delay(MCSDL_DELAY_1US);
    MCSDL_GPIO_SDA_SET_OUTPUT_ISC(0);mcsdl_delay(MCSDL_DELAY_1US);
