...
#define HW_VERSION_EMPTY       0x00	   //When firmware is empty.
...
#define REPORT_MT(touch_number, x, y, amplitude, width) \
do {     \
...
//    if (ret < 0)
//    {
//        pr_err("[TSP] melfas_ts_probe: i2c_master_send() failed\n [%d]", ret);
//        return 0;
//    }
...
static int read_input_info(struct melfas_ts_data *ts, u8 *val,u8 start_addr, int read_length)
{
...
{
	return melfas_i2c_read(ts->client, TS_READ_VERSION_ADDR, 2, val);
}
...
static int firmware_update(struct melfas_ts_data *ts)
{
	...
	if (i == P2_MAX_INFO_READ_FAIL) {
		...
		pr_info("[TSP] check_firmware H/W:[%x],S/W:[%x],Core:[%x],Private:[%x],Public:[%x]",
		...
		if(ret > 0 || fw_ver[0] == HW_VERSION_EMPTY ){
			...
			if( !(fw_ver[0] == HW_VER1 && fw_ver[1] == FW_VER1) )
				fw_isp_update |= true;
			...
			if(touch_id == 0){			// GFF S-mac
				if( fw_ver[0] < GFS_HW_BASE_VER || (fw_ver[0] == GFS_HW_BASE_VER &&  fw_ver[1] < GFS_SW_BASE_VER) )
					fw_isp_update |= true;
			}else if(touch_id == 1){	// G2 -Morins
				if( fw_ver[0] < G2M_HW_BASE_VER || (fw_ver[0] == G2M_HW_BASE_VER &&  fw_ver[1] < G2M_SW_BASE_VER) )
					fw_isp_update |= true;
			}else if(touch_id == 2){	// GFF Digitec
				if( fw_ver[0] < GFD_HW_BASE_VER || (fw_ver[0] == GFD_HW_BASE_VER &&  fw_ver[1] < GFD_SW_BASE_VER) )
					fw_isp_update |= true;
			}else if(touch_id == 3){	// G2 Wintec
	//			if( fw_ver[0] < G2W_HW_BASE_VER || (fw_ver[0] == G2W_HW_BASE_VER &&  fw_ver[1] < G2W_SW_BASE_VER) )
	//				fw_update |= true;
				pr_info("[TSP] touch_id = 3 TSP pannel is detached!");
				return 0;
			}
		}
		...
		if (!fw_isp_update && system_rev >= 2 && !fw_isc_update) {
			...
			if((touch_id == 0 && fw_ver[4] < GFS_PRIVATE_VER)
					|| (touch_id == 1 && fw_ver[4] < G2M_PRIVATE_VER)
					|| (touch_id == 2 && fw_ver[4] < GFD_PRIVATE_VER)
...
#ifdef TEST_FW_UPDATE
			pr_info("[TSP] TEST_FW_UPDATE! ");
			fw_isp_update	|= true;	// force f/w upload
			touch_id	= 4;	// Test f/w upload
#endif
...
//	fw_isp_update |= true;
	...
	if(!fw_isp_update && !fw_isc_update ){
		...
		pr_info("[TSP] ISP D/L mode %s & ISC D/L mode %s [%x]"
					,fw_isp_update?"ON":"OFF",fw_isc_update?"ON":"OFF",fw_isc_update);
	}
	...
	if(fw_isp_update){
		...
		if (ret){
			pr_info("[TSP] ISC D/W Fail & Try ISP mode D/L [%d]\n", ret);
			ret = mcsdl_download_binary_data(touch_id);
			if (ret)
				pr_info("[TSP] ISC & ISP SET Download ALL Fail [%d]\n", ret);
		}
	}
	...
	if(ret)
		...
		pr_info("[TSP] After D/L - H/W:[%x],S/W:[%x],Core:[%x],Private:[%x],Public:[%x]",
					fw_ver[0],fw_ver[1],fw_ver[3],fw_ver[4],fw_ver[5]);
...
static void release_all_fingers(struct melfas_ts_data *ts)
{
	...
	for(i=0; i<MELFAS_MAX_TOUCH; i++) {
		...
		REPORT_MT(i, g_Mtouch_info[i].posX, g_Mtouch_info[i].posY,
...
static void reset_tsp(struct melfas_ts_data *ts)
{
	...
	pr_info("[TSP] reset_tsp & TA/USB %sconnect\n",ta_status?"":"dis");
	melfas_i2c_write(ts->client, (char *)buf, 2);
	msleep(150);
...
static void melfas_ts_read_input(struct melfas_ts_data *ts)
{
	...
	int touchStatus = 0, touchPosition = 0, posX = 0, posY = 0, width = 0, strength = 0,pre_strength = 0;
	...
	if(read_num > 0){
		...
		if(touchStatus == 0x0F ) { // ESD Detection
			pr_info("[TSP] TSP ESD Detection [%x]",buf[0]);
			reset_tsp(ts);
			return ;
		}else if(touchStatus == 0x1F ){ //RF Noise Detection On
			pr_info("[TSP] TSP RF Noise Detection On [%x]",buf[0]);
			return ;
		}
		...
		for(i=0; i<read_num; i=i+6){
			...
			pre_strength = g_Mtouch_info[FingerID].strength;
			g_Mtouch_info[FingerID].strength = (buf[i+4] == 0 ? 0:buf[i+4]+20);
			g_Mtouch_info[FingerID].width= buf[i+5]/7+3;

#if SHOW_COORD
		if((buf[i] & 0x80)==0){ //release
			pr_info("[TSP] Release [%2d], X: [%4d], Y: [%3d], Strength: %d width: %d\n",
				FingerID, g_Mtouch_info[FingerID].posX, g_Mtouch_info[FingerID].posY, 
				g_Mtouch_info[FingerID].strength, g_Mtouch_info[FingerID].width);
		}else{					//press
			if(pre_strength == -1){
				pr_info("[TSP] Press   [%2d], X: [%4d], Y: [%3d], Strength: %d width: %d\n",
					FingerID, g_Mtouch_info[FingerID].posX, g_Mtouch_info[FingerID].posY, 
					g_Mtouch_info[FingerID].strength, g_Mtouch_info[FingerID].width);
			}
		}
		...
		schedule_delayed_work(&ts->dvfs_work, msecs_to_jiffies(TOUCH_BOOSTER_TIME));
				       ...
				       char *buf)
{
	...
	if(ts->touch_id == 0)
		...
		return snprintf(buf, PAGE_SIZE,	"G2M_%2.2Xx%2.2X\n", fw_ver[0], fw_ver[1]);
	else if(ts->touch_id == 2)
		return snprintf(buf, PAGE_SIZE,	"GFD_%2.2Xx%2.2X\n", fw_ver[0], fw_ver[1]);
	else if(ts->touch_id == 3)
		return snprintf(buf, PAGE_SIZE,	"G2W_%2.2Xx%2.2X\n", fw_ver[0], fw_ver[1]);
	else
		return snprintf(buf, PAGE_SIZE,	"MEL_%2.2Xx%2.2X\n", fw_ver[0], fw_ver[1]);
}
			      ...
			      char *buf)
{
	...
	if (!status) {
		...
		pr_info("[TSP] all_refer_show func & check_debug_data error[%d]",
					status);
...
static DEVICE_ATTR(set_index_ref, S_IWUSR | S_IWGRP, NULL, set_index_reference);
...
static int melfas_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (ts == NULL)
	{
		pr_err("[TSP] melfas_ts_probe: failed to create a state of melfas-ts\n");
		ret = -ENOMEM;
...
err_detect_failed:
	pr_err("[TSP] melfas-ts: err_detect failed\n");
	kfree(ts);
...
{
	struct melfas_ts_data *ts = i2c_get_clientdata(client);
	int ta_status = ts->charging_status,ret =0 ;
	char buf[2];
