...
int mc_control_active_state(int val)
{
	...
	if (mc_is_modem_on()) {
		...
		dev_info(global_mc->dev, "Host Active:%d\n",
				gpio_get_value(global_mc->gpio_host_active));
	}
...
int mc_prepare_resume(int ms_time)
{
	...
	if (val == HOST_WUP_LEVEL) {
		dev_err(mc->dev, "svn HOST_WUP:high!\n");
		return MC_HOST_HIGH;
	}
...
int mc_reconnect_gpio(void)
{
	...
	dev_info(mc->dev, "try reconnection...\n");
...
static void kernel_upload(struct modemctl *mc)
{
	...
	if (mc->cpcrash_flag)
		...
		kernel_sec_set_upload_cause(0xFF);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!strncmp(buf, "dump", 4)) {
		dev_err(mc->dev, "%s - LTE Forced Dump!!!\n", __func__);
		dump_start(mc);
	}
...
static DEVICE_ATTR(wakeup, S_IRUGO | S_IWUSR | S_IWGRP,
		show_wakeup, store_wakeup);
...
static void mc_work(struct work_struct *work_arg)
{
	...
	if (ret < 0) {
		dev_err(mc->dev, "failed to get modem active value\n");
		return;
	}

	if (ret) {
		if (mc->cpcrash_flag == 1)
			dump_end(mc);
	} else
		dump_start(mc);
}
...
static irqreturn_t modemctl_resume_irq(int irq, void *dev_id)
{
	...
	if (val != HOST_WUP_LEVEL) {
		...
		if (gpio_get_value(mc->gpio_host_active) &&
				!gpio_get_value(mc->gpio_slave_wakeup)) {
			gpio_set_value(mc->gpio_slave_wakeup, 1);
			pr_info("> S- WUP %d", gpio_get_value(mc->gpio_slave_wakeup));
		}
