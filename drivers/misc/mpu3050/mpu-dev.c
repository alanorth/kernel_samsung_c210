...
void mpu3050_early_suspend(struct early_suspend *h)
{
	struct mpu_private_data *mpu = container_of(h,
			struct mpu_private_data, early_suspend);
	struct mldl_cfg *mldl_cfg = &mpu->mldl_cfg;
	struct i2c_adapter *accel_adapter;
	struct i2c_adapter *compass_adapter;
	struct i2c_adapter *pressure_adapter;
			...
			short *temp_avg)
{
	...
	for (j = 0; j < 3; j++) {
		if (RMS[j] > (int)DEF_RMS_THRESH_SQ * total_count) {
			pr_info("%s :%s-Gyro RMS (%d) exceeded threshold (%.4f)\n",
...
#elif defined(CONFIG_MACH_P8LTE_REV00) || defined(CONFIG_MACH_P8LTE_REV01)
	x *= (-1);
	/* y *= (-1); */
	/* z *= (-1); */
	return sprintf(buf, "%d, %d, %d\n", y, x, z);
		...
		const struct i2c_device_id *devid)
{
	...
	if (res)
		pr_info(KERN_ERR "%s: cound not register accelerometer sensor device(%d).\n", __func__, res);
