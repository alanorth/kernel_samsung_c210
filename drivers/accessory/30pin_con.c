...
static int acc_get_adc_value(struct s3c_adc_client *padc)
{
	int adc_data;
...
static int connector_detect_change(struct acc_con_info *acc)
{
	int i;
	u32 adc = 0, adc_sum = 0;
	u32 adc_buff[5] = {0};
	...
	for (i = 0; i < 5; i++) {
		adc_val = acc_get_adc_value(acc->padc);
		adc_buff[i] = adc_val;
		adc_sum += adc_buff[i];
		if (i == 0) {
			adc_min = adc_buff[0];
...
{
	if (NULL == acc->pdata->get_acc_state) {
	...
	if (acc->pdata->get_acc_state()) {
		...
		ACC_CONDEV_DBG("[30PIN] dock station atached!!!");
...
static void acc_delay_work(struct work_struct *work)
{
	...
	if (retval) {
		ACC_CONDEV_DBG("failed to initialize the accessory det irq");
		goto err_irq_dock;
	}
...
static void acc_id_delay_work(struct work_struct *work)
{
	...
	dock_state = acc->pdata->get_dock_state();
	...
	if (dock_state) {
		...
		adc_val = connector_detect_change(acc);
