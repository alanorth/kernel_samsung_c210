...
#define CP_PORT		1	/* CMC220 LTE modem use port 1 */
#else
...
extern int usb_host_phy_off(void);
...
#if defined(CONFIG_SAMSUNG_PHONE_SVNET) || defined(CONFIG_SAMSUNG_LTE)
...
#else defined(CONFIG_UMTS_LINK_HSIC)
extern int umts_is_modem_on(void);
extern int umts_is_host_wakeup(void);
extern int umts_set_link_active(int val);
extern int umts_set_slave_wakeup(int val);
#endif

static void s5pv210_start_ehc(void);
static int s5pv210_stop_ehc(void);
...
static int usb_host_phy_power_init(struct platform_device *pdev)
{
	int retval = 0;
...
void s5pv210_hsic_port1_power(int enable)
{
	...
	printk("[%s] ...power %s port1(USB3503 HSIC port)...\n", __func__, enable ? "up" : "down");
		(void) ehci_hub_control(ehci_to_hcd(ehci),
				enable ? SetPortFeature : ClearPortFeature,
				USB_PORT_FEAT_POWER,
				USB3503_PORT, NULL, 0);
...
EXPORT_SYMBOL(s5pv210_hsic_port1_power);
#endif
...
#else
	printk(KERN_DEBUG "USB_PM ehci rt susp\n");
	s5pv210_stop_ehc(); /* turn off phy */
#endif
	return 0;
}
...
#else
	printk(KERN_DEBUG "USB_PM ehci rt resume\n");
	s5pv210_start_ehc(); /* turn on phy */
	rc=1; /* to do reset-resume */
#endif
	if (rc) {
		struct usb_hcd *hcd = s5pv210_hcd;
		struct ehci_hcd *ehci = hcd_to_ehci(hcd);
...
static void s5pv210_wait_for_cp_resume(struct usb_hcd *hcd)
{
	...
	umts_set_link_active(1);
#endif
	do {
		msleep(10);
		val32 = ehci_readl(ehci, portsc);
	} while (++retry_cnt < RETRY_CNT_LIMIT && !(val32 & PORT_CONNECT));
	pr_info("%s: port status: 0x%x, retry_cnt = %d\n", __func__,
			val32, retry_cnt);
}
...
#if defined(CONFIG_USB_OHCI_HCD)
		if (ohci_hcd) {
			pm_runtime_forbid(&s3c_device_usb_ohci.dev);
...
#if defined(CONFIG_USB_OHCI_HCD)
		if (ohci_hcd) {
			retval = usb_add_hcd(ohci_hcd, IRQ_UHOST,
					IRQF_DISABLED | IRQF_SHARED);
			if (retval < 0) {
				dev_err(dev, "OHCI power on fail\n");
				goto exit;
			}
			pm_runtime_allow(&s3c_device_usb_ohci.dev);
		}
#endif
		/*HSIC IPC control the ACTIVE_STATE*/
#if defined(CONFIG_SAMSUNG_PHONE_SVNET) || defined(CONFIG_SAMSUNG_LTE)
...
static int ehci_hcd_s5pv210_drv_probe(struct platform_device *pdev)
{
	...
	if (usb_host_phy_power_init(pdev) < 0) {
