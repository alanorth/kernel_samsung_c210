...
static struct android_usb_product *get_product(struct android_dev *dev);
static void update_dev_desc(struct android_dev *dev);
...
static int android_bind_config(struct usb_configuration *c)
{
	...
	printk(KERN_DEBUG "usb: android_bind_config\n");
	dev->config = c;
...
static struct android_usb_product *get_product(struct android_dev *dev)
{
	...
	printk(KERN_ERR "usb: Can't find product !\n");
	return 0;
}
...
static int android_bind(struct usb_composite_dev *cdev)
{
	...
	printk(KERN_INFO "usb: android_bind\n");
	...
	if (ret) {
		printk(KERN_ERR "usb: usb_add_config failed\n");
		return ret;
	}
	...
	/* Samsung kies uses below number */
	...
	printk(KERN_DEBUG "usb: android_bind PID=0x%x\n", cdev->desc.idProduct);
...
void android_register_function(struct android_usb_function *f)
{
	...
	printk(KERN_INFO "usb: android_register_function %s\n", f->name);
	...
	if (dev && should_bind_functions(dev)) {
		...
		device_desc.idVendor = __constant_cpu_to_le16(
				get_vendor_id(dev));
		device_desc.idProduct = __constant_cpu_to_le16(
				get_product_id(dev));
...
void android_enable_function(struct usb_function *f, int enable)
{
	...
	printk(KERN_DEBUG "usb: %s->disabled(%d) enable(%d)\n",
			f->name, f->disabled, enable);
	if (!!f->disabled != disable) {
		...
		p = get_product(dev);
		printk(KERN_DEBUG "usb: product=0x%p\n", p);
		dev->cdev->product = p;
		printk(KERN_DEBUG "usb: force reset [v=0x%x p=0x%x]\n",
...
static int android_probe(struct platform_device *pdev)
{
	...
	printk(KERN_INFO "usb: android_probe pdata: %p\n", pdata);
...
static int __init init(void)
{
	...
	printk(KERN_INFO "usb: android init\n");
