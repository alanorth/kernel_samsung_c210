...
static int smb136_get_charging_state(void)
{
	...
	pr_info("%s: POWER_SUPPLY_STATUS_UNKNOWN, 0x31h(0x%02x), 0x36h(0x%02x), 0x39h(0x%02x)\n", __func__,
		command_a_data, statue_e_data, statue_h_data);
	return (int)status;
}
...
static int smb136_get_charging_current(void)
{
	...
	switch(data >> 5) {
	case 0:	get_current = 500;	break;
	case 1:	get_current = 650;	break;
	case 2:	get_current = 750;	break;
	case 3:	get_current = 850;	break;
	case 4:	get_current = 950;	break;
	case 5:	get_current = 1100;	break;
	case 6:	get_current = 1300;	break;
	case 7:	get_current = 1500;	break;
	default:
		get_current = 500;
		break;
	}
...
static int smb136_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	chg->callbacks = kzalloc(sizeof(struct smb_charger_callbacks), GFP_KERNEL);
	if (!chg->callbacks) {
