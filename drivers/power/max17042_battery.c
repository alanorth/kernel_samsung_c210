...
static int fg_read_temp(void)
{
	...
	if (fg_check_battery_present()) {
		...
		if (data[1]&(0x1 << 7)) {
			...
			/* Adjust temperature over 47, only for SDI battery type */
			if (fg_get_battery_type() == SDI_BATTERY_TYPE) {
				if(temper >= 47000 && temper < 60000)
					temper = temper * SDI_TRIM1_1 / 100 - SDI_TRIM1_2;
				else if(temper >= 60000)
					temper = temper * SDI_TRIM2_1 / 100 - 51000;
			}
		}
	} else
		temper = 20000;
...
int fg_check_cap_corruption(void)
{
	...
	if (chip->pdata->check_jig_status()) {
		pr_info("%s : Return by Using Jig(%d)\n", __func__, chip->pdata->check_jig_status());
		return 0;
	}
	...
	if ((((vfsoc+5) < chip->info.previous_vfsoc) || (vfsoc > (chip->info.previous_vfsoc+5)))
		...
		pr_info("[FG_Recovery] (B) VfSOC(%d), prevVfSOC(%d), RepSOC(%d), prevRepSOC(%d), MixCap(%d), prevMixCap(%d),VfOCV(0x%04x, %d)\n",
			vfsoc,
			chip->info.previous_vfsoc,
			repsoc, chip->info.previous_repsoc,
			(mixcap/2),
			(chip->info.previous_mixcap/2),
			vfocv,
			pr_vfocv);
...
void prevent_early_poweroff(int vcell, int *fg_soc)
{
	...
	pr_info("%s : soc=%d%%(0x%04x), vcell=%d\n", __func__, repsoc, repsoc_data, vcell);
...
/* P4W & P8 needs one more level */
    ...
    defined(CONFIG_MACH_P8_REV00) || defined(CONFIG_MACH_P8_REV01) || defined(CONFIG_MACH_P8LTE_REV00)
		case 5:
			if (level == 1)
...
int low_batt_compensation(int fg_soc, int fg_vcell, int fg_current)
{
	...
	if (!chip->info.low_batt_comp_flag
		...
		if (fg_min_current < CURRENT_RANGE_MAX) {
			if (fg_soc >= 2 && fg_vcell < get_low_batt_threshold(CURRENT_RANGE_MAX_NUM, 1, fg_min_current))
				add_low_batt_comp_cnt(CURRENT_RANGE_MAX_NUM, 1);
			else if (fg_soc >= 4 && fg_vcell < get_low_batt_threshold(CURRENT_RANGE_MAX_NUM, 3, fg_min_current))
...
#if defined(CONFIG_MACH_P4W_REV00) || defined(CONFIG_MACH_P4W_REV01) || \
    defined(CONFIG_MACH_P8_REV00) || defined(CONFIG_MACH_P8_REV01) || defined(CONFIG_MACH_P8LTE_REV00)
		else if (fg_min_current >= CURRENT_RANGE5 && fg_min_current < CURRENT_RANGE4) {
...
#endif
		else if (fg_min_current >= CURRENT_RANGE4 && fg_min_current < CURRENT_RANGE3) {
			if (fg_soc >= 2 && fg_vcell < get_low_batt_threshold(3, 1, fg_min_current))
				...
				bCntReset = 1;
		} else if (fg_min_current >= CURRENT_RANGE3 && fg_min_current < CURRENT_RANGE2) {
			if (fg_soc >= 2 && fg_vcell < get_low_batt_threshold(2, 1, fg_min_current))
				...
				bCntReset = 1;
		} else if (fg_min_current >= CURRENT_RANGE2 && fg_min_current < CURRENT_RANGE1) {
...
#if defined(CONFIG_MACH_P4W_REV00) || defined(CONFIG_MACH_P4W_REV01) ||  \
    defined(CONFIG_MACH_P2_REV00) || defined(CONFIG_MACH_P2_REV01) || defined(CONFIG_MACH_P2_REV02)
...
static void fg_set_battery_type(void)
{
	...
	if (!chip->info.check_start_vol) {
		if (chip->info.battery_type == SDI_BATTERY_TYPE)
			chip->info.check_start_vol = chip->pdata->sdi_low_bat_comp_start_vol;
		else if (chip->info.battery_type == ATL_BATTERY_TYPE)
			chip->info.check_start_vol = chip->pdata->atl_low_bat_comp_start_vol;
	}
...
static int fg_read_avsoc(void)
{
	...
	if (fg_i2c_read(client, AVSOC_REG, data, 2) < 0) {
