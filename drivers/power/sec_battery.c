...
#if defined(CONFIG_MACH_P2_REV00) || defined(CONFIG_MACH_P2LTE_REV00)
#define P2_CHARGING_FEATURE_01	/* SMB136 + MAX17042, Cable detect by PMIC */
#endif

#if defined(CONFIG_MACH_P2_REV01) || defined(CONFIG_MACH_P2_REV02)
...
#define P4_CHARGING_FEATURE_01	/* MAX8903 + MAX17042, use TA_nCON */
#endif

#if defined(CONFIG_MACH_P4W_REV02)
#define P4_CHARGING_FEATURE_02	/* SMB347 + MAX17042, use TA_nCON */
#endif
...
static int get_cached_charging_status(struct battery_data *battery)
{
	...
	pr_info("PMIC detect : %s\n", ta_con ? "TA connected" : "TA NOT connected");
	return ta_con;
}

static int check_ta_conn(struct battery_data *battery)
{
#if defined(P2_CHARGING_FEATURE_01)	/* P2 : Detect by PMIC status */
	struct power_supply *psy = power_supply_get_by_name("max8997-charger");
	union power_supply_propval value;
	u32 ta_con = 0;
	...
	msleep(100); /* Wait stable DCIN state */
	if (psy->get_property) {
		psy->get_property(psy, POWER_SUPPLY_PROP_STATUS, &value);
		if (value.intval == POWER_SUPPLY_STATUS_CHARGING)
			ta_con = 1;
	...
	pr_debug("PMIC detect : %s\n", ta_con ? "TA connected" : "TA NOT connected");
	return ta_con;
#else	/* P2(rev0.1~) P4C, P8 : Detect by GPIO(TA_nCONNECTED) */
	u32 value;
	...
	if ((system_rev >= 2) && (system_rev <= 5))
		value = !value;
#endif
	pr_debug("Charger detect : %s\n", value ? "TA NOT connected" : "TA connected");
	return !value;
#endif
}
...
{
	battery->charging_mode_booting = battery->pdata->check_lp_charging_boot();
	pr_info("%s : charging_mode_booting(%d)\n", __func__, battery->charging_mode_booting);
}
#endif
...
static void sec_set_charging(struct battery_data *battery, int charger_type)
{
	...
	pr_info("%s: Set charging current as %dmA.\n", __func__, battery->info.charging_current);
}
#endif
...
static irqreturn_t sec_TA_nCHG_interrupt_handler(int irq, void *arg)
{
	...
	pr_info("%s(%d)\n", __func__, gpio_get_value(battery->pdata->charger.fullcharge_line));

	disable_irq_nosync(irq);
	cancel_delayed_work(&battery->fullcharging_work);
	schedule_delayed_work(&battery->fullcharging_work, msecs_to_jiffies(300));
	wake_lock_timeout(&battery->fullcharge_wake_lock, HZ * 30);

	return IRQ_HANDLED;
}

#if !defined(P2_CHARGING_FEATURE_01)
static irqreturn_t sec_TA_nCon_interrupt_handler(int irq, void *arg)
{
	struct battery_data *battery = (struct battery_data *)arg;
	pr_info("%s(%d)\n", __func__, gpio_get_value(battery->pdata->charger.connect_line));
...
static int sec_get_bat_level(struct power_supply *bat_ps)
{
	...
	if (!battery->pdata->check_jig_status() && !max17042_chip_data->info.low_batt_comp_flag) {
...
/* P4-Creative does not set full flag by force */
	...
	if (battery->info.charging_source == CHARGER_AC &&
		...
		if (is_over_abs_time(battery)) {
			...
			pr_info("%s: Charging time is over. Active abs timer. VCELL(%d), SOC(%d)\n", __func__, fg_vcell, fg_soc);
	...
	if ((fg_vcell <= battery->pdata->recharge_voltage) ||
		...
		if (battery->info.batt_is_full
			...
			if (++battery->recharging_cnt > 1) {
				pr_info("%s: Recharging start(%d ? %d).\n", __func__, fg_vcell, battery->pdata->recharge_voltage);
...
{
	struct power_supply *internal_psy = power_supply_get_by_name("max8997-charger");
	union power_supply_propval value;
	int ret;
	...
	if (internal_psy) {
		...
		ret = internal_psy->set_property(internal_psy, POWER_SUPPLY_PROP_STATUS, &value);
		if (ret)
...
static void sec_set_chg_current(struct battery_data *battery, int set_current)
{
	...
	if (battery->current_cable_status == CHARGER_AC) {
		...
		if (battery->pdata->get_charging_current)
			battery->info.charging_current = battery->pdata->get_charging_current();
	} else {
...
static void sec_set_chg_en(struct battery_data *battery, int enable)
{
#if defined(P2_CHARGING_FEATURE_01) || defined(P2_CHARGING_FEATURE_02) || defined(P4_CHARGING_FEATURE_02)
	/* Disable internal charger(MAX8997), because of using external charger */
	...
	pr_info("%s: External charger is %s", __func__, (enable ? "enabled." : "disabled."));
	...
	if (battery_temp >= temp_high_threshold) {
		...
		if (health == POWER_SUPPLY_HEALTH_OVERHEAT ||
			...
			pr_info("battery recovery(%d, %d~%d), charging available.",
				battery_temp, temp_low_recovery, temp_high_recovery);
	...
	case SUSPEND_LOCK:
		if (sscanf(buf, "%d\n", &mode) == 1) {
			...
			if (mode)
				...
				wake_lock_timeout(&debug_batterydata->wake_lock_for_dev, HZ / 2);
			ret = count;
		}
		break;
...
{
	struct battery_data *battery = (struct battery_data *)arg;
	pr_debug("%s(%d)", __func__, gpio_get_value(max17042_chip_data->pdata->fuel_alert_line));
...
static void fullcharging_work_handler(struct work_struct *work)
{
	...
	if (gpio_get_value(battery->pdata->charger.fullcharge_line) == 1) {
		...
		/* Check charger state */
		if (battery->pdata->get_charging_state) {
			if (battery->pdata->get_charging_state() == POWER_SUPPLY_STATUS_UNKNOWN) {
				pr_info("Disconnect cable and check charger.\n");
				if (battery->pmic_cable_state == 0) {
					battery->cable_detect_source = 2;	/* Cable detect from PMIC */
					sec_get_cable_status(battery);
					sec_cable_changed(battery);
		...
		battery->cable_detect_source = 0;	/* Cable detect from default */
	}
...
static int __devinit sec_bat_probe(struct platform_device *pdev)
{
	...
	battery->psy_battery.num_properties = ARRAY_SIZE(sec_battery_properties);
	...
	if (request_irq(battery->charging_irq, sec_TA_nCHG_interrupt_handler, trigger,
			"TA_nCHG intr", battery)) {
	...
	trigger = IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING;
	if (request_irq(battery->connect_irq, sec_TA_nCon_interrupt_handler, trigger,
