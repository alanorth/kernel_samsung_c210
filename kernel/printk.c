 ...
 * In above case, log_buf size is 256KB and its base address is 0x45000000 physically
 * Actually, *(int *)(base - 8) is log_magic and *(int *)(base - 4) is log_ptr
 * So we reserve (size + 8) bytes from (base - 8)
 */
...
static char *sec_log_buf_for_getlog;
...
extern void map_nonchached_task_log_buf(unsigned long va_task_log_buf);
#endif
...
static int __init sec_log_setup(char *str)
{
...
#else
		log_buf_iodesc[0].virtual = (unsigned long)S3C_VA_KLOG_BUF;
...
#ifdef CONFIG_SEC_DEBUG_SCHED_LOG
		map_nonchached_task_log_buf((unsigned long)(S3C_VA_KLOG_BUF + 0x100000 + size));
#endif
#endif
		sec_log_buf_for_getlog = phys_to_virt(base);
		sec_log_size = size;
	...
	sec_getlog_supply_kloginfo(sec_log_buf_for_getlog);
