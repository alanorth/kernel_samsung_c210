...
static void s5p_rp_backup_sbuf(void);
static void s5p_rp_restore_sbuf(void);
...
static void s5p_rp_flush_ibuf(void)
{
	...
	s5p_rp.ibuf_fill_size[0] = 0;
	s5p_rp.ibuf_fill_size[1] = 0;
#ifdef _USE_START_WITH_BUF0_
...
static void s5p_rp_fill_ibuf(void)
{
	unsigned long fill_size;

	if (!s5p_rp.wbuf_pos)		/* wbuf empty? */
		return;

	if (s5p_rp.wbuf_pos >= s5p_rp.ibuf_size) {
		fill_size = s5p_rp.ibuf_size;
		s5p_rp.wbuf_pos -= fill_size;
	} else {
		fill_size = s5p_rp.wbuf_pos;
		memset(&s5p_rp.wbuf[fill_size], 0xFF, s5p_rp.ibuf_size - fill_size);
		s5p_rp.wbuf_pos = 0;
	}

	if (s5p_rp.ibuf_next == 0) {
		memcpy(s5p_rp.sbuf, s5p_rp.ibuf0, s5p_rp.ibuf_size);
		memcpy(s5p_rp.ibuf0, s5p_rp.wbuf, s5p_rp.ibuf_size);
		s5pdbg("Fill IBUF0 (%lu)\n", fill_size);
		s5p_rp.ibuf_empty[0] = 0;
		s5p_rp.ibuf_next = 1;
		s5p_rp.sbuf_fill_size = s5p_rp.ibuf_fill_size[0];
		s5p_rp.ibuf_fill_size[0] = s5p_rp.ibuf_fill_size[1] + fill_size;
	} else {
		memcpy(s5p_rp.sbuf, s5p_rp.ibuf1, s5p_rp.ibuf_size);
		memcpy(s5p_rp.ibuf1, s5p_rp.wbuf, s5p_rp.ibuf_size);
		s5pdbg("Fill IBUF1 (%lu)\n", fill_size);
		s5p_rp.ibuf_empty[1] = 0;
		s5p_rp.ibuf_next = 0;
		s5p_rp.sbuf_fill_size = s5p_rp.ibuf_fill_size[1];
		s5p_rp.ibuf_fill_size[1] = s5p_rp.ibuf_fill_size[0] + fill_size;
	}

	if (s5p_rp.wbuf_pos)
		memcpy(s5p_rp.wbuf, &s5p_rp.wbuf[s5p_rp.ibuf_size], s5p_rp.wbuf_pos);
}

static void s5p_rp_set_stream_size(void)
{
	if (s5p_rp.wbuf_pos)	/* Leave stream size max, if data is available */
		return;
				...
				size_t size, loff_t * pos)
{
	...
	if (s5p_rp.wbuf_pos < s5p_rp.ibuf_size) {
		...
		while (!s5p_rp.early_suspend_entered && !s5p_rp.auto_paused &&
...
static long s5p_rp_ioctl(struct file *file,
			unsigned int cmd, unsigned long arg)
{
	...
	case S5P_RP_SEND_EOS:
		...
		if (s5p_rp.wbuf_fill_size == 0) {	/* No data? */
			...
			s5p_rp_fill_ibuf();		/* Fill IBUF0 */
#ifdef _USE_START_WITH_BUF0_
			s5p_rp.ibuf_req_skip = 0;
#else
			if (s5p_rp.ibuf_empty[s5p_rp.ibuf_next])
				s5p_rp_fill_ibuf();	/* Fill IBUF1 */
#endif
			s5p_rp_set_stream_size();
			s5pdbg("Start RP decoding!!\n");
			writel(0x00000000, s5p_rp.commbox + RP_PENDING);
			s5p_rp_is_running = 1;
	...
	case S5P_RP_RESUME_EOS:
		...
		s5p_rp_flush_obuf();
		if (s5p_rp.restart_after_resume) {
			s5p_rp_fill_ibuf();		/* Fill IBUF0 */
#ifdef _USE_START_WITH_BUF0_
			s5p_rp.ibuf_req_skip = 0;
#else
			...
			if (s5p_rp.decoding_started) {
				if (s5p_rp.ibuf_empty[0] && s5p_rp.ibuf_empty[1]) {
					if (s5p_rp.wait_for_eos) {
						...
						printk(KERN_WARNING "S5P_RP: Auto-Pause (buffer underrun)\n");
						s5p_rp_pause();
...
static long s5p_rp_ctrl_ioctl(struct file *file,
				unsigned int cmd, unsigned long arg)
{
	int ret_val = 0;
...
static int s5p_rp_prepare_fw_buff(struct device *dev)
{
	...
	s5p_rp.sbuf = dma_alloc_writecombine(0, _SBUF_SIZE_,
			   (dma_addr_t *)&s5p_rp.sbuf_pa, GFP_KERNEL);
...
static int s5p_rp_remove_fw_buff(void)
{
	...
	dma_free_writecombine(0, _SBUF_SIZE_, s5p_rp.sbuf, s5p_rp.sbuf_pa);
...
static void s5p_rp_sbuf_fill(unsigned char *buf, unsigned long buf_size)
{
	memcpy(&s5p_rp.sbuf[s5p_rp.sbuf_fill_size], buf, buf_size);
	s5p_rp.sbuf_fill_size += buf_size;

	s5pdbg("SBUF Fill (0x%08lX) Total = 0x%08lX\n",
		buf_size, s5p_rp.sbuf_fill_size);
}

static void s5p_rp_sbuf_fill_ibuf_frac(int ibuf_index)
{
	unsigned long frac;

	if (!s5p_rp.ibuf_fill_size[ibuf_index])		/* empty? */
		return;

	frac = s5p_rp.ibuf_fill_size[ibuf_index] % s5p_rp.ibuf_size;
	if (ibuf_index) {
		s5pdbg("Backup IBUF1\n");
		s5p_rp_sbuf_fill(s5p_rp.ibuf1, frac ? frac : s5p_rp.ibuf_size);
	} else {
		s5pdbg("Backup IBUF0\n");
		s5p_rp_sbuf_fill(s5p_rp.ibuf0, frac ? frac : s5p_rp.ibuf_size);
	}
}

static void s5p_rp_sbuf_fill_ibuf_skip_rp(int ibuf_index, unsigned long rp_frac)
{
	unsigned long len;

	len = s5p_rp.ibuf_size - rp_frac;
	if (ibuf_index) {
		s5pdbg("Backup IBUF1\n");
		s5p_rp_sbuf_fill(&s5p_rp.ibuf1[rp_frac], len);
	} else {
		s5pdbg("Backup IBUF0\n");
		s5p_rp_sbuf_fill(&s5p_rp.ibuf0[rp_frac], len);
	}
}

static void s5p_rp_sbuf_fill_ibuf_frac_skip_rp(int ibuf_index, unsigned long rp_frac)
{
	unsigned long frac, len;

	frac = s5p_rp.ibuf_fill_size[ibuf_index] % s5p_rp.ibuf_size;
	len = frac ? frac - rp_frac : s5p_rp.ibuf_size - rp_frac;
	if (ibuf_index) {
		s5pdbg("Backup IBUF1\n");
		s5p_rp_sbuf_fill(&s5p_rp.ibuf1[rp_frac], len);
	} else {
...
static void s5p_rp_sbuf_fill_ibuf(void)
{
	unsigned long rp_read, rp_frac;

	rp_read = readl(s5p_rp.commbox + RP_READ_BITSTREAM_SIZE);
	rp_frac = rp_read % s5p_rp.ibuf_size;

	if (rp_read < s5p_rp.sbuf_fill_size) {	/* SBUF */
		s5pdbg("Backup SBUF\n");
		s5p_rp.sbuf_fill_size = s5p_rp.ibuf_size - rp_frac;
		memcpy(s5p_rp.sbuf, &s5p_rp.sbuf[rp_frac], s5p_rp.sbuf_fill_size);

		if (s5p_rp.ibuf_fill_size[0] < s5p_rp.ibuf_fill_size[1]) {
			s5p_rp_sbuf_fill_ibuf_frac(0);
			s5p_rp_sbuf_fill_ibuf_frac(1);
		} else {
			s5p_rp_sbuf_fill_ibuf_frac(1);
			s5p_rp_sbuf_fill_ibuf_frac(0);
		}
	} else {
		s5p_rp.sbuf_fill_size = 0;	/* Clear sbuf */
		if (s5p_rp.ibuf_fill_size[0] < s5p_rp.ibuf_fill_size[1]) {
			if (rp_read < s5p_rp.ibuf_fill_size[0]) {
				s5p_rp_sbuf_fill_ibuf_skip_rp(0, rp_frac);
				s5p_rp_sbuf_fill_ibuf_frac(1);
			} else {		/* Skip IBUF0 */
				s5p_rp_sbuf_fill_ibuf_frac_skip_rp(1, rp_frac);
			}
		} else {
			if (rp_read < s5p_rp.ibuf_fill_size[1]) {
				s5p_rp_sbuf_fill_ibuf_skip_rp(1, rp_frac);
				s5p_rp_sbuf_fill_ibuf_frac(0);
			} else {		/* Skip IBUF1 */
				s5p_rp_sbuf_fill_ibuf_frac_skip_rp(0, rp_frac);
			}
		}
	}
}
...
static void s5p_rp_backup_sbuf(void)
{
	if (!s5p_rp.ibuf_fill_size[0] && !s5p_rp.ibuf_fill_size[1])
		...
		s5p_rp_sbuf_fill_ibuf();

	if (s5p_rp.wbuf_pos) {
		s5pdbg("Backup WBUF\n");
		s5p_rp_sbuf_fill(s5p_rp.wbuf, s5p_rp.wbuf_pos);
	}

	s5pdbg("WBUF fill_size   = 0x%08lX\n", s5p_rp.wbuf_fill_size);
	s5pdbg("Backup size      = 0x%08lX\n", s5p_rp.sbuf_fill_size);

	if (s5p_rp.decoding_started) {
		s5pdbg("restart_after_resume required\n");
		s5p_rp.restart_after_resume = 1;
	}
}

static void s5p_rp_restore_sbuf(void)
{
	if (s5p_rp.sbuf_fill_size) {
		memcpy(s5p_rp.wbuf, s5p_rp.sbuf, s5p_rp.sbuf_fill_size);
		s5p_rp.wbuf_pos = s5p_rp.sbuf_fill_size;
		s5p_rp.wbuf_fill_size = s5p_rp.sbuf_fill_size;
	}
}
